...
exports.index = function(req, res){
    ...
    //proses async
    Q.all([
        Room.count(), //total data
        Room.find().skip(skip).limit(limit) //jumlah data
    ])
...
exports.search = function(req,res){
    ...
    Room.find({ name: { $regex: req.query.value, $options: 'i' } }).exec(function(err, rooms){
...
exports.update = function(req, res){
     ...
     Room.findOne({ _id: req.params.id }).exec(function(err, room){
        ...
        updated.save(function(err){
            ...
            res.status(200).json({ massage: 'Room Updated', room: updated });
        });
    });
}
