...
exports.index = function(req, res){
    ...
    //proses async
    Q.all([
        Medicine.count(), //total data
        Medicine.find().skip(skip).limit(limit) //jumlah data
    ])
...
exports.search = function(req,res){
    ...
    Medicine.find({ name: { $regex: req.query.value, $options: 'i' } }).exec(function(err, medicines){
...
exports.update = function(req, res){
     ...
     Medicine.findOne({ _id: req.params.id }).exec(function(err, medicine){
        ...
        updated.save(function(err){
            ...
            res.status(200).json({ massage: 'Medicine Updated', medicine: updated });
        });
    });
}
